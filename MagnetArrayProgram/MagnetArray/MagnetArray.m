...
f = figure('Units', 'normalized',...
    ...
    'Name','MagnetArrarys');    % name of the figure

% add a tool bar to hold two push buttons: About and Help
addToolbar = uitoolbar(f); % creat the tool bar
cdat_about = imread('about.bmp','bmp'); % load UNB logo image as the About image
...
% Add labels 
ylabel(handles.magneticFieldZ,'|B|','fontsize',8);
...
% Add labels
ylabel(handles.magneticFieldY,'Z','fontsize',8);
xlabel(handles.magneticFieldY,'|B|','fontsize',8);
...
%% Function to plot B_Y and B_Z
    function magneticFieldVectorPlot(hObject,event_data)
        ...
        else    %otherwise, plot Y cross section
        ...
        if(isYPlot) %if it is to plot Z cross section
            ...
            B_Z = sqrt(Bz.^2+By.^2);
            B(find(B==Inf))=NaN; % ignor if it is B==Inf
            ...
            plot(Z,B_Y,'b'); % plot
            ylabel(handles.magneticFieldY,'Z','fontsize',8); %add labels
            xlabel(handles.magneticFieldY,'|B|','fontsize',8);
            grid on;
            %%%
...
%% function to export contour values if exportContourBtn clicked
    function exportContourBtnClicked(hObject,event_data)
        ...
        savefile = strcat(path,file)
        separator = NaN(1,50);% use 'NaN' to seperate variables
        save(savefile,'Zm','separator','Ym','separator','B','-ASCII')
